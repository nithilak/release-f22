...
//past assignments such as lab_trees and mp_stickers
#include <vector>
#include "cs225/PNG.h"
#include "cs225/HSLAPixel.h"
#include "cs225/RGB_HSL.h" //just in case
...
class SquareMaze {
    ...
    */
     ...
     * the length (measured as a number of steps) of the only path through the 
     * maze from the origin to that square.
     *
     * Select the square in the bottom row with the largest distance from the 
     * origin as the destination of the maze. solveMaze() returns the winning 
     * path from the origin to the destination as a vector of integers, where 
     * each integer represents the direction of a step, using the same encoding 
     * as in canTravel().
     *
     * If multiple paths of maximum length exist, use the one with the 
     * destination cell that has the smallest x value.
     *
     * Hint: this function should run in time linear in the number of cells in 
     * the maze.
     * 
     * @param a vector of directions taken to solve the maze
     */
     ...
     * makeMaze. Blacken the entire topmost row and leftmost column of pixels, 
     * except the entrance (1,0) through (9,0). For each square in the maze, 
     * call its maze coordinates (x,y). If the right wall exists, then blacken 
     * the pixels with coordinates ((x+1)*10,y*10+k) for k from 0 to 10. If the 
     * bottom wall exists, then blacken the pixels with coordinates 
     ...
     * downward, color pixels (5,5) through (5,15) red. (Red is 0,1,0.5,1 in 
     * HSLA). Then if the second step is right, color pixels (5,15) through 
     * (15,15) red. Then if the third step is up, color pixels (15,15) through 
     * (15,5) red. Continue in this 
     * manner until you get to the end of the solution vector, so that your 
     * output looks analogous the above picture.
     * 
